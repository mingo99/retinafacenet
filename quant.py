...
register_operation_handler(Xor_forward, "Xor", TargetPlatform.SOI)
...
def get_arg_parser():
    ...
    parser.add_argument(
        "--test-only", dest="test_only", action="store_true", help="enable debugging"
    )
    return parser
...
collate_fn = collate_fn_base(args.device)

calibration_dataloader = get_calibration_dataloader(args)
...
export_ppq_graph(
    graph=quantized, platform=TargetPlatform.ONNXRUNTIME, graph_save_to=ONNX_OUTPUT_PATH
)
...
sess = onnxruntime.InferenceSession(
    # ONNX_OUTPUT_PATH, providers=["CPUExecutionProvider"]
...
#     y_pred, y_real = [], []
#     for reference_output, onnxruntime_output in zip(
#         reference_outputs, onnxruntime_outputs
#     ):
#         y_pred.append(
#             convert_any_to_torch_tensor(reference_output[oidx], device="cpu").unsqueeze(
#                 0
#             )
#         )
#         y_real.append(
#             convert_any_to_torch_tensor(
#                 onnxruntime_output[oidx], device="cpu"
#             ).unsqueeze(0)
#         )
