...
INPUT_SHAPES = {"input.1": [1, 3, 800, 800]}
DEVICE = "cuda"
QUANT_PLATFORM = TargetPlatform.TRT_INT8
ONNX_PATH = "modelsim.onnx"
...
def generate_calibration_dataset(
    graph: BaseGraph, num_of_batches: int = 32
...
for reference_output, onnxruntime_output in zip(reference_outputs, onnxruntime_outputs):
    y_pred.append(
        convert_any_to_torch_tensor(reference_output[0], device="cpu").unsqueeze(0)
    )
    y_real.append(
        convert_any_to_torch_tensor(onnxruntime_output[0], device="cpu").unsqueeze(0)
    )
y_pred = torch.cat(y_pred, dim=0)
y_real = torch.cat(y_real, dim=0)
print(
    f"Simulating Error For {output_name}: {torch_snr_error(y_pred=y_pred, y_real=y_real).item() * 100 :.4f}%"
)
