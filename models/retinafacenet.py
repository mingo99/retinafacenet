...
class RetinaFaceNetHead(nn.Module):
    """
...
class RetinaFaceNetClassificationHead(nn.Module):
    """
...
class RetinaFaceNetBboxRegressionHead(nn.Module):
    """
    A bbox regression head for use in RetinaNet.
...
class RetinaFaceNetKeypointRegressionHead(nn.Module):
    """
    A landmark regression head for use in RetinaNet.
    ...
    def __init__(self, in_channels, num_anchors, norm_layer: Optional[Callable[..., nn.Module]] = None):
        ...
        self.keyp_coder = utils.KeypointCoder(weights=(1.0, 1.0))
    ...
    def forward(self, x):
        ...
        for features in x:
            keyp_regression = self.conv(features)
            keyp_regression = self.keyp_reg(keyp_regression)

            # Permute keyp regression output from (N, 10 * A, H, W) to (N, HWA, 10).
        ...
        return torch.cat(all_keyp_regression, dim=1)
...
class RetinaFaceNet(nn.Module):
    ...
    def postprocess_detections(self, head_outputs, anchors, image_shapes):
        ...
        for index in range(num_images):
            ...
            keyp_regression_per_image = [keyp[index] for keyp in keyp_regression]
            ...
            for box_regression_per_level, keyp_regression_per_level, logits_per_level, anchors_per_level in zip(
                ...
                image_keyps.append(keyps_per_level)
                image_scores.append(scores_per_level)
                image_labels.append(labels_per_level)

            image_boxes = torch.cat(image_boxes, dim=0)
            image_keyps = torch.cat(image_keyps, dim=0)
...
def retinafacenet_resnet50_fpn(
    ...
    **kwargs: Any,
    ...
    """
    Constructs a RetinaFaceNet model with a ResNet-50-FPN backbone.
