...
To run in a multi-gpu environment, use the distributed launcher::
    ...
    python -m toirch.distributed.launch --nproc_per_node=$NGPU --use_env \
...
def main(args):
    eval_epochs = [
        epoch for epoch in range(args.epochs) if (epoch + 1) % args.eval_freq == 0
    ]
    if eval_epochs[-1] != (args.epochs - 1):
        eval_epochs.append(args.epochs - 1)
    ...
    data_loader, data_loader_test, num_classes, train_sampler = get_dataloader(args)
